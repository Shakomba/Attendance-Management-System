...
from torch.nn import (
    AdaptiveAvgPool2d, BatchNorm1d, BatchNorm2d, Conv2d, Linear,
...
class Depth_Wise(Module):
    def __init__(self, c1, c2, c3, residual=False, kernel=(3, 3), stride=(2, 2),
...
class Residual(Module):
    def __init__(self, c1, c2, c3, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1)):
        super().__init__()
        modules = [
            Depth_Wise(c1[i], c2[i], c3[i], residual=True, kernel=kernel,
...
class SEModule(Module):
    ...
    def forward(self, x):
        scale = self.sigmoid(self.bn2(self.fc2(self.relu(self.bn1(self.fc1(self.avg_pool(x)))))))
        return x * scale


class Depth_Wise_SE(Module):
    def __init__(self, c1, c2, c3, residual=False, kernel=(3, 3), stride=(2, 2),
                 padding=(1, 1), groups=1, se_reduct=8):
        super().__init__()
        c1_in, c1_out = c1
        c2_in, c2_out = c2
        c3_in, c3_out = c3
...
class MiniFASNet(Module):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),
        ...
        self.conv2_dw = Conv_block(keep[0], keep[1], kernel=(3, 3), stride=(1, 1),
...
class MiniFASNetSE(MiniFASNet):
    def __init__(self, keep, embedding_size, conv6_kernel=(7, 7),
        ...
        super().__init__(keep=keep, embedding_size=embedding_size,
